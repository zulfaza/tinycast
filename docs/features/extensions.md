...
`stream/web` and stream-state predicates), `net`'s IP predicates and HTTP socket bridge, `util`, `events`,
`buffer` (`Buffer`, `Blob`), `url`, `querystring`, `punycode`, `assert`, `string_decoder`, `timers`,
`async_hooks`, `diagnostics_channel`. Every other built-in resolves to a stub that throws only when used, so a
